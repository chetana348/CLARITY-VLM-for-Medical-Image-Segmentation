# Modified from Bert by Google and Hugging Face
...
from models.layers import Linear_d
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        layer_output = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, ao1, ao2
        )
    ...
    def feed_forward_chunk(self, ao1, ao2):
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # todo
        # self.layer1 = nn.adfnmald * 4
        # self.layer2 = nn.adfnmald * 4
        # self.layer3 = nn.adfnmald * 4
    ...
    def forward(
        ...
        # Validate idx and mapping

        if idx == 0:
            start_layer = 0                         #mapping: List[int] = [1, 6, 8, 10, 12],  
        else:        
            start_layer = mapping[idx - 1]
        end_layer = mapping[idx]
        for layer_idx in range(start_layer, end_layer):
            layer_module = self.layer[layer_idx]
...
class BertPooler(nn.Module):
    ...
    def relprop(self, cam, **kwargs):
        ...
        # print(f"act : {cam.shape}")
        cam = self.dense.relprop(cam, **kwargs)
        # print(f"dense : {cam.shape}")
        # cam = cam.unsqueeze(1)
        # cam = self.pool.relprop(cam, **kwargs)
        # print(f"pool : {cam.shape}")
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str):
            ...
            print(f"self.transform_act_fn: {self.transform_act_fn}")
        else:
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = Linear_d(config.hidden_size, config.vocab_size, bias=False)
    ...
    def relprop(self, cam, mask_id=None, **kwargs):
        cam = self.decoder.relprop(cam, **kwargs)
        # print(f"decoder : {cam.shape}")
        cam = self.transform.relprop(cam, **kwargs)
        # print(f"transform : {cam.shape}")
        cam = cam[:, mask_id,:]
        # print(f"final cam : {cam.shape}")
        return cam
...
class BertOnlyMLMHead(nn.Module):
    ...
    def forward(self, sequence_output: torch.Tensor) -> torch.Tensor:
        ...
        # print(f"prediction_scores : {prediction_scores.shape}")
        return prediction_scores
    
    def relprop(self, cam, mask_id=None, **kwargs):
        cam = self.predictions.relprop(cam, mask_id, **kwargs)
        return cam
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        else: #idx =1, input parameter input_ids
            embedding_output = input_ids #output from previous stage
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.config.tokenizer = BertTokenizer.from_pretrained("google-bert/bert-base-uncased")
    ...
    def relprop(self, cam=None, mask_id=None, **kwargs):
        cam = self.cls.relprop(cam, mask_id, **kwargs, )
        # print(f"cls: {cam.shape}")
        # cam = self.dropout.relprop(cam, **kwargs)
